...
Adding a language to CortexWard means shipping a package that registers a
:class:`LanguageProvider` under the ``cortexward.languages`` entry-point group
(see :mod:`cortexward.plugins.registry`) — the core never changes.
"""
...
from cortexward.ports.code_graph import CodeGraph


@runtime_checkable
class LanguageProvider(Protocol):
    """Adapts a single programming language to the CortexWard core.
    ...
    def parse(self, root: Path) -> CodeGraph:
        """Build a :class:`~cortexward.ports.code_graph.CodeGraph` for ``root``."""
        ...
