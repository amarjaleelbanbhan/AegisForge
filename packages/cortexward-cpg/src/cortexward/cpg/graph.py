"""An in-memory :class:`~cortexward.ports.code_graph.CodeGraph` implementation.

This is the reference graph engine: language-agnostic, backed by adjacency
lists over the four edge kinds in :mod:`cortexward.cpg.model`. A
:class:`GraphBuilder` accumulates nodes and edges (as a
:class:`~cortexward.ports.language_provider.LanguageProvider` walks a parse
...
class GraphBuilder:
    ...
    Not thread-safe and not itself a :class:`~cortexward.ports.code_graph.
